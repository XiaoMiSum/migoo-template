 ...
 */
export const addBeginAndEndTime = (params: any, dateRange: [...any], propName: string) => {
  // 必须传入参数
  if (!dateRange) {
    return params
  }
  ...
  // 设置参数
  if (dateRange[0]) {
    params['begin' + propName] =
      (typeof dateRange[0] === 'object' ? parseTime(dateRange[0], '{y}-{m}-{d}') : dateRange[0]) +
      ' 00:00:00'
  }
  if (dateRange[1]) {
    params['end' + propName] =
      (typeof dateRange[1] === 'object' ? parseTime(dateRange[1], '{y}-{m}-{d}') : dateRange[1]) +
      ' 23:59:59'
  }
  return params
}
...
export const rounding = (amount: number | string, fixed?: number) => {
  return typeof amount === 'number' ? Number(amount).toFixed(fixed || 2) : amount
}
